...
- **两个小动作**：按 <kbd>Q</kbd> 交替播放「踢腿」和「害羞捂脸」（捂脸时会闭眼、脸红、耳朵耷拉下来）
...
| 操作 | 反应 |
| --- | --- |
| <kbd>Q</kbd> 键 | **交替触发两个动作**：踢腿 → 害羞捂脸 → 踢腿……循环切换 |
...
src/index.html     页面壳
src/renderer.js    纸偶合成渲染 + 动作状态机（idle/kick/drag/shy）+ 热键循环
tools/             图像处理管线（需 Python + Pillow + numpy）
assets/            零件 PNG + manifest.js（由管线生成）
```
